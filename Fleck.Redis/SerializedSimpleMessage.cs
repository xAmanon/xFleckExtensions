...
{
    public class SerializedSimpleMessage
    {
        ...
        public SerializedSimpleMessage(IReadOnlyList<SerializedMessage> messages)
        {
        ...
        /// Initializes a new instance of the <see cref="SerializedHubMessage"/> class.
        /// </summary>
        /// <param name="message">The hub message for the cache. This will be serialized with an <see cref="ISimpleProtocol"/> in <see cref="GetSerializedMessage"/> to get the message's serialized representation.</param>
        public SerializedSimpleMessage(Message message)
        {
            Message = message;
        }

        /// <summary>
        /// Gets the serialized representation of the <see cref="HubMessage"/> using the specified <see cref="ISimpleProtocol"/>.
        /// </summary>
        ...
        public string GetSerializedMessage(ISimpleProtocol protocol)
        {
            lock (_lock)
            {
                if (!TryGetCached(protocol.Name, out var serialized))
                {
                    ...
                    serialized = protocol.GetMessageText(Message);
        ...
        private void SetCache(string protocolName, string serialized)
        {
        ...
        private bool TryGetCached(string protocolName, out string result)
        {
